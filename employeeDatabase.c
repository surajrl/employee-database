...
static struct Employee read_employee(void)
{
   ...
   while (job_done != 1)
   {
      fprintf(stdout, "Enter your job: ");
      read_line(stdin, job, MAX_NAME_LENGTH);

      if (strlen(job) == 0)
      {
         fprintf(stdout, "Invalid job.\n");
         continue;
      }
...
static void menu_delete_employee(void)
{
   ...
   for (i = 0; i < no_employees; i++)
   {
      ...
      {
         fprintf(stdout, "Employee not found.\n");
         continue;
      }  
...
static void read_employee_database ( char *file_name )
{
   ...
   for (i = 0; i < 10; i++)
   {
      ...
      if (strlen(name) == 0)
      {
         fprintf(stdout, "Error: Invalid name found in the database.");
         exit(EXIT_FAILURE);
      }
      ...
      {
         fprintf(stdout, "Error: Invalid sex found in the database.\n");
         exit(EXIT_FAILURE);
      }
      ...
      if (age <= 0)
      {
         fprintf(stdout, "Error: Invalid age found in the database.\n");
         exit(EXIT_FAILURE);
      }
      ...
      if (strlen(job) == 0)
      {
         fprintf(stdout, "Error: Invalid job found in the database..\n");
         exit(EXIT_FAILURE);
      }
...
int main ( int argc, char *argv[] )
{
   ...
   if ( argc != 1 && argc != 2 )
   {
      fprintf ( stdout, "Usage: %s [<database-file>]\n", argv[0] );
      exit(-1);
   }
   ...
   for(;;)
   {
      ...
      fprintf ( stdout, "\nOptions:\n" );
      fprintf ( stdout, "%d: Add new employee to database\n", ADD_CODE );
      fprintf ( stdout, "%d: Delete employee from database\n", DELETE_CODE );
      fprintf ( stdout, "%d: Print database to screen\n", PRINT_CODE );
      fprintf ( stdout, "%d: Exit database program\n", EXIT_CODE );
      fprintf ( stdout, "\nEnter option: " );
      ...
      if ( result != 1 )
      {
	 fprintf ( stdout, "corrupted menu choice\n" );
	 continue;
      }
	 ...
	 break;
         ...
         default:
	 fprintf ( stdout, "illegal choice %d\n", choice );
	 break;
      }
